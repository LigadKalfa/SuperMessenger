...
extension AllChatsController{
    func setupData() {
        ...
        if let context = delegate?.persistentContainer.viewContext {
            ...
            AllChatsController.createMessageWithText(text: "Bijo is my sweet little lover", friend: boy, minutesAgo: 2, context: context)
            ...
            AllChatsController.createMessageWithText(text: "Give me Hachapuri!", friend: bijo, minutesAgo: 10, context: context)
            
            let mama = NSEntityDescription.insertNewObject(forEntityName: "Friend", into: context) as! Friend
            mama.name = "Mamaliga"
            mama.profileImageName = "Mama.jpeg"

            AllChatsController.createMessageWithText(text: "Oh sweet son!", friend: mama, minutesAgo: 60 * 24, context: context)

            let pops = NSEntityDescription.insertNewObject(forEntityName: "Friend", into: context) as! Friend
            pops.name = "Don Pepe"
            pops.profileImageName = "Pops.jpeg"
            
            AllChatsController.createMessageWithText(text: "Oh sweet son!", friend: pops, minutesAgo: 8 * 60 * 24, context: context)
*/
    ...
    private func createGirlMessagesWithContext(context: NSManagedObjectContext){
        ...
        AllChatsController.createMessageWithText(text: "Hello, my name is Bijo's girl, wanna dance tonight?? <3 i have a" +
            "lot to tell you about how my day was going and how every body love me at work!", friend: girl, minutesAgo: 3, context: context)
        AllChatsController.createMessageWithText(text: "and tonight? -_-", friend: girl, minutesAgo: 2, context: context)
        AllChatsController.createMessageWithText(text: "seriosly i will always love bijo bijoshvily", friend: girl, minutesAgo: 0, context: context)
        AllChatsController.createMessageWithText(text: "Do you love me do you do you?", friend: girl, minutesAgo: 0, context: context)

        //response message
        AllChatsController.createMessageWithText(text: "Well, dear, im in love with you.", friend: girl, minutesAgo: 0, context: context, isSender: true)
    }
    
    static func createMessageWithText(text: String, friend: Friend, minutesAgo: Double,context: NSManagedObjectContext,
    ...
    func loadData() {
        ...
        if let context = delegate?.persistentContainer.viewContext {
            ...
            if let friends = AllChatsController.fetchFriends() {
    ...
    static func fetchFriends() -> [Friend]? {
        let delegate = UIApplication.shared.delegate as? AppDelegate
